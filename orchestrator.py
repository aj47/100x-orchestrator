...
def save_tasks(tasks_data):
    """Save all agent data to the database."""
    try:
        for agent_id, agent_data in tasks_data.get("agents", {}).items():
            existing_agent = get_agent(agent_id)
            agent_data['progress_history'] = json.dumps(agent_data.get('progress_history', []))
            agent_data['thought_history'] = json.dumps(agent_data.get('thought_history', []))
            if existing_agent:
        ...
        logging.error(f"Error saving agent data to database: {e}", exc_info=True)
